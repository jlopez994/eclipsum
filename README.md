...
- **Multi-eclipse browser** (its own tab): upcoming eclipses ~8 years ahead and a paginated archive ~25 years back, all engine-generated on device; the catalog (bundled + Remote Config) curates labels and paths on top. Each eclipse keeps its own spot and alerts. Picking a past one opens it in **archive mode**: that day's map, path, local times and actual cloud cover (Open-Meteo ERA5 archive), with alerts off.
- **Eclipses from here**: from the map sheet, every eclipse visible from your spot (past and future) with its local obscuration — tap one to jump to it keeping the spot.
...
lib/
  ...
  bandGeo.ts             Bundled path pack, whole browsable window (scripts/genBandPack.ts)
  spotEclipses.ts        Eclipses visible from one spot (local engine sweep)
...
components/
  screens/               MapScreen, EclipsesScreen, AlertsScreen, SettingsScreen, EclipseModeScreen
  ...
  SpotEclipses.tsx       "Eclipses from here" modal (past + upcoming from the spot)
...
scripts/
  ...
  genBandPack.ts         Bundles paths for the whole browsable window → lib/bandGeo.ts
  genBand.ts             One bundled path by id (manual/regional ranges)
  syncBands.ts           Generates missing paths in the RC template
  bandWalk.ts            Shared path walker (used by genBandPack and syncBands)
...
| `eclipse_catalog` | JSON of extra eclipses (with optional `band` path) without a release. Safety net: serves paths to old builds and to installs whose bundled window has rolled past |
...
**Bundled path pack** (offline on first run, zero maintenance): `npx tsx scripts/genBandPack.ts` regenerates `lib/bandGeo.ts` with every total/annular path in the browsable window (~25 years back + ~8 ahead, ~50 paths ≈ 240 KB). Incremental — already-bundled paths are kept, only missing ones are computed (history never changes). Partials have no central path by nature. `genBand.ts --id <id>` remains for one-off regional ranges. This route ships with the APK, so it requires a release; RC covers the gap for builds that never update.
